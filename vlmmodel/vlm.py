...
def preprocess_v1(
    ...
    #     print(prompt,tokenizer_image_token(prompt, tokenizer, return_tensors='pt').size())
    ...
    for conversation, target in zip(conversations, targets):
        ...
        rounds = conversation.split(conv.sep2)
        #0929 bug!!!! conversation token length is more that rounds[0],len(rounds[1])==0,the gap between conversation and rounds[0] is 2
        # print("ruonds")
        # print(len(conversation),tokenizer_image_token(conversation, tokenizer, return_tensors='pt').size())

        # print(len(rounds[0]),tokenizer_image_token(rounds[0], tokenizer, return_tensors='pt').size())
        # print(len(rounds[1]),tokenizer_image_token(rounds[1], tokenizer, return_tensors='pt').size())
        ## 0929 暂时改为2
        cur_len = 2
        target[:cur_len] = IGNORE_INDEX
        for i, rou in enumerate(rounds):
            ...
            # 这里的bug，长度对不上导致标签一直为0，rou的长度=part0+part1+sep-1，-1这个很奇怪
            # print(len(parts[0]),len(parts[1]),len(rou),len(sep))
            # print(tokenizer_image_token(parts[0], tokenizer, return_tensors='pt').size())
            # print(tokenizer_image_token(parts[1], tokenizer, return_tensors='pt').size())
            # print(tokenizer_image_token(rou, tokenizer, return_tensors='pt').size())
            # print(tokenizer_image_token(sep, tokenizer, return_tensors='pt').size())
            ...
            if has_image:
                ...
                round_len = len(tokenizer(rou).input_ids)
                instruction_len = len(tokenizer(parts[0]).input_ids) - 2
            print(len(target),instruction_len,cur_len,round_len)
...
def make_supervised_data_module_clients(tokenizer: transformers.PreTrainedTokenizer,
    ...
    return client_training_datasets,global_test_dataset,data_collator
...
def init_vlm_model_test(training_args,model_args,data_args):
    global local_rank
